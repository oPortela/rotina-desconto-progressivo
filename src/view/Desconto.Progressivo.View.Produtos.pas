...
begin
  if Application.MessageBox('Deseja realmente excluir este produto?', 'Atenção', MB_YESNO + MB_ICONQUESTION) <> mrYes then
    abort;
  try
    ...
    ShowMessage('Produto ' + CodigoProduto + ' excluído com sucesso!');
...
begin
  ...
  if not dmProdutos.SQLProdutos.Eof then
    ...
    DescricaoCampanha := 'Descrição não encontrada';
